...
class Event extends \CommonBundle\Component\Hydrator\Hydrator
{
    ...
    protected function doExtract($object = null)
    {
        ...
        if (sizeof($event->getOptions()) == 0) {
            $data['prices']['price_members'] = number_format($event->getPriceMembers(), 2);
            $data['prices']['price_non_members'] = $event->isOnlyMembers() ? '' : number_format($event->getPriceNonMembers(), 2);
        } else {
