...
class Ticket extends \CommonBundle\Component\Hydrator\Hydrator
{
    ...
    protected function doHydrate(array $data, $object = null)
    {
        ...
        if ($data['is_guest']) {
            $person = null;
            $guestInfo = new GuestInfo($data['guest_form']['guest_first_name'], $data['guest_form']['guest_last_name'], $data['guest_form']['guest_email']);
            $this->getEntityManager()->persist($guestInfo);
        } else {
            $person = $this->getEntityManager()
                ->getRepository('CommonBundle\Entity\User\Person\Academic')
                ->findOneById($formData['person_form']['person_id']);
            $guestInfo = null;
        }
